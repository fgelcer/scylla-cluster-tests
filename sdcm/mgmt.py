...
class SCTool():
    ...
    def run(self, cmd, is_verify_errorless_result=False, parse_table_res=True, is_multiple_tables=False,  # pylint: disable=too-many-arguments
        ...
        try:
            # output_ascii_table=True,
            # if output_ascii_table:
            #     # setting SCTOOL_ASCII_TABLES so output of sctool will be in ascii
            #     res = self._remoter_run(cmd="sudo bash -cxe 'SCTOOL_ASCII_TABLES=TRUE\nsctool {}'".format(cmd))
            # else:
